...
@section('content')
    <div class="max-w-4xl mx-auto mt-8">
        <div class="bg-white shadow-sm rounded-xl p-6 border border-gray-100">
            <h2 class="text-lg font-semibold text-gray-800 mb-6">Tambah Program Donasi</h2>
            ...
            <form action="{{ route('admin.programs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <select name="penggalang_id"
                        ...
                        @foreach ($penggalangs as $penggalang)
                            <option value="{{ $penggalang->id }}"
                                {{ old('penggalang_id') == $penggalang->id ? 'selected' : '' }}>
                                {{ $penggalang->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="target_amount" value="{{ old('target_amount') }}"
                        class="w-full border-0 border-b border-gray-300 focus:border-primary focus:ring-0"
                        placeholder="Contoh: 10000000" min="0">
                </div>

                <!-- Hidden collected_amount -->
                <input type="hidden" name="collected_amount" value="{{ old('collected_amount', 0) }}">
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>Nonaktif</option>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    ...
                    <button type="submit"
                        class="px-5 py-2 bg-primary text-white rounded-lg hover:bg-green-700 transition">Simpan</button>
                </div>
            </form>
        </div>
    </div>
