        ...
        <div class="bg-white p-6 shadow-md rounded-lg mx-auto max-w-lg mt-4">
            ...
            <div class="text-center mb-6">
                ...
                @elseif ($donation->status === 'pending')
                    ...
                    <p class="text-sm text-gray-500 mt-2">Kode Pembayaran Anda: <span
                            class="font-semibold">{{ $donation->donation_code }}</span></p>
            ...
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span
                        class="font-medium capitalize text-{{ $donation->status === 'success' ? 'green' : ($donation->status === 'pending' ? 'yellow' : 'red') }}-600">
                        {{ $donation->status }}
                    </span>
                </div>
                @if ($donation->status === 'pending' && $donation->status_change_at)
                    <div class="flex justify-between text-xs text-red-500 pt-1 border-t">
                        ...
                        <span
                            class="font-semibold">{{ $donation->status_change_at->addHours(24)->translatedFormat('d F Y, H:i') }}
                            WIB</span>
                    </div>
                @endif
            </div>
            ...
            @if ($isDeletable)
                <div class="mt-4 p-3 bg-red-100 border border-red-400 rounded-lg text-red-700 text-sm">
                    ...
                    Data donasi ini (Gagal/Expired) akan **dihapus otomatis dalam waktu dekat** karena telah melewati batas
                    24 jam.
                </div>
            @endif


            {{-- kalau belum bayar/gagal maka muncul tombol ini --}}
            @if (in_array($donation->status, ['pending', 'failed', 'expired']))
                <a href="{{ route('donation.pay', $donation->donation_code) }}"
                    class="w-full block py-3 rounded-lg font-bold text-center bg-blue-600 text-white hover:bg-blue-700 transition shadow-md mt-4">
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">Status:
                                    <span
                                        class="capitalize text-{{ $donor->status === 'success' ? 'green' : ($donor->status === 'pending' ? 'yellow' : 'red') }}-500 font-semibold">{{ $donor->status }}</span>
                                </p>
